...
// Calculate 10-Year minus 2-Year Yield Spread
$cm10YMinus2Y = [];
foreach ($cm10YData as $date => $rate10Y) {
    if (isset($cm2YData[$date])) {
        $cm10YMinus2Y[$date] = $rate10Y - $cm2YData[$date];
    }
}
...
<script>
    ...
    var trace7 = {
        x: ' . json_encode(array_keys($cm10YMinus2Y)) . ',
        y: ' . json_encode(array_values($cm10YMinus2Y)) . ',
        type: "scatter",
    ...
    var layout = {
        title: "Yield Spreads, Mortgage Rates, MOVE Index, STLFSI4, and Recessions",
        xaxis: {
            title: "Date"
        },
        yaxis: {
